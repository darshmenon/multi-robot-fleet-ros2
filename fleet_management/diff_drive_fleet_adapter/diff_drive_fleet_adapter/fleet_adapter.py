...
#
# Adapted from rmf_demos_fleet_adapter/fleet_adapter.py: this orchestration
# layer (RobotAdapter, update loop, ROS 2 connections) is fleet-adapter
# generic and kept as-is; only the RobotAPI construction below is
# diff_drive_robot-specific (direct ROS 2 instead of a REST fleet manager).
...
def main(argv=sys.argv):
    ...
    # Parse the yaml in Python to get rmf_fleet-level settings
    ...
    # Initialize robot API for this fleet: direct ROS 2 (Nav2 + tf), no
    # REST fleet manager. update_period/map_name come from rmf_fleet, not
    # a fleet_manager block (diff_drive_robot has none).
    rmf_fleet_yaml = config_yaml['rmf_fleet']
    update_period = 1.0 / rmf_fleet_yaml.get(
        'robot_state_update_frequency', 10.0
    )
    map_name = rmf_fleet_yaml.get('map_name', 'L1')
    api = RobotAPI(node, map_name)
